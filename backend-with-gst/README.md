...
A comprehensive job processing and report generation system with role-based authentication, admin dashboard, and bulk data processing capabilities.
...
│   │   ├── controllers/
│   │   │   ├── adminController.js # Admin CSV download endpoints
...
│   │   ├── models/
│   │   │   ├── ClientModel.js     # Client data model with roles
│   │   │   └── JobProcessingModel.js # Job processing model
│   │   ├── queues/
│   │   │   └── job.queue.js       # BullMQ queue configuration
│   │   ├── routes/
...
- `GET /api/jobs/:id/download` - Download processed report ZIP
...
4. **Data Enrichment:** Python scripts process directorship data
...
- **Input:** Excel files with directorship data
- **Processing:** Python scripts handle GST validation, DIN lookups
...
```javascript
{
  ...
  jobId: String,
  clientId: ObjectId,
  jobStatus: String, // 'pending', 'processing', 'completed', 'completed_with_errors', 'failed'
