...
const GLOBAL_FAILED_HEADER = "Ref No,Candidate Name,PAN,jobId,error,Timestamp\n";
...
const GLOBAL_PROCESSED_HEADER = "Ref No,Candidate Name,PAN,DIN,GST,Status,Timestamp\n";
        ...
        if (fs.existsSync(jobFailedRowsPath)) {
            ...
            if (allLines.length === 0) {
                ...
                if (!/timestamp/i.test(currentHeader)) {
                    ...
                    const existingData = allLines.slice(1).map(line => line.replace(/\r/g, '').trim()).filter(Boolean);
                    const updatedData = existingData.map(line => line + ',' + timestamp);
                    const newContent = GLOBAL_FAILED_HEADER + (updatedData.length ? updatedData.join('\n') + '\n' : '');
        ...
        await new Promise((resolve, reject) => {
            ...
            process.stdout.on("data", (data) => {
                ...
                if (output.includes("Converting to PDF")) {
                    JobProcessingModel.findByIdAndUpdate(jobId, { $inc: { reportsGenerated: 1 } }).catch(err => console.error("Failed to update reportsGenerated:", err));
                }
            });
